...
import { getPaymentsByStatus, getProductsByStallId, getStallBySlug, sumAmountUsd } from "@/lib/mock-data";

const MERCHANT_STALL_SLUG = "golden-hour-bakery";

export default function MerchantDashboardPage() {
  const stall = getStallBySlug(MERCHANT_STALL_SLUG)!;
  const products = getProductsByStallId(stall.id);
  const { bg } = accentClasses(stall.accent);
  const settledPayments = getPaymentsByStatus("settled");
  const settledToday = sumAmountUsd(settledPayments);
        ...
        <div className="rounded-[28px] border border-border-soft bg-surface-solid p-[26px] lg:col-span-2">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <p className="text-[14px] text-ink-faint">
              Today · ${settledToday.toFixed(2)} settled
            </p>
          </div>
          <div className="mt-4">

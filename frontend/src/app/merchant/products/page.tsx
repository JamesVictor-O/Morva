...
import { getStallBySlug, MERCHANT_PRODUCTS } from "@/lib/mock-data";
...
export default function ProductsPage() {
  ...
  const [products, setProducts] = useState<MerchantProduct[]>(MERCHANT_PRODUCTS);
  ...
  function saveEdit(id: string) {
    ...
    setProducts((prev) =>
      prev.map((p) => (p.id === id && Number.isFinite(parsed) && parsed > 0 ? { ...p, priceUsd: parsed } : p))
    );
    setEditingId(null);
  }
  ...
  function commitRestock(id: string) {
    const amount = restockAmountFor(id);
    setProducts((prev) =>
      prev.map((p) => (p.id === id ? { ...p, stock: p.stock + amount, status: "in-stock" } : p))
    );
      ...
      <Topbar
        ...
        right={
          <>
            ...
            <AvatarTile label="GH" accent={stall.accent} size="lg" className="rounded-full" />
          </>
        }
      />
    ...
    <div className={`flex flex-wrap items-center gap-4 py-4 ${withDivider ? "border-t border-divider" : ""}`}>
      ...
      <div className="ml-auto flex flex-none items-center gap-4">
        {editing ? (
          ...
          <button onClick={onCommitRestock} className="text-[14px] font-semibold text-ink">
            Restock
          </button>
        ) : (

...
import { getPaymentsByStatus, getStallBySlug, sumAmountUsd, PAYMENTS, WEEKLY_STATS } from "@/lib/mock-data";
...
export default function PaymentsPage() {
  const stall = getStallBySlug("golden-hour-bakery")!;
  const [filter, setFilter] = useState<Filter>("all");

  const settled = getPaymentsByStatus("settled");
  const clearing = getPaymentsByStatus("pending");
  const settledTodayUsd = sumAmountUsd(settled);
  const settlingNowUsd = sumAmountUsd(clearing);
  ...
  return (
    <AppShell variant="merchant">
      <Topbar
        ...
        right={<AvatarTile label="GH" accent={stall.accent} size="lg" className="rounded-full" />}
      />

      <main className="px-5 py-8 sm:px-8 lg:px-[34px] lg:py-10">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard label="Settled today" value={`$${settledTodayUsd.toFixed(2)}`}>
            Across {settled.length} sales
          </StatCard>
          <StatCard label="This week" value={`$${WEEKLY_STATS.weekUsd.toFixed(2)}`}>
            {WEEKLY_STATS.salesThisWeek} sales
          </StatCard>
        ...
        <div className="mt-6 flex items-center gap-4 rounded-[24px] border border-border-soft bg-surface-solid p-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[16px] font-semibold text-ink">
              Settling to {truncateAddress(stall.payoutAddress)}
            </p>
            <p className="truncate text-[13px] text-ink-faint">
              USDC · automatically, the moment each sale clears
            </p>
          </div>
...
function PaymentRow({ payment, withDivider }: { payment: Payment; withDivider: boolean }) {
  return (

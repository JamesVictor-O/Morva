...
export function PlazaPageClient({ stalls, featured }: { stalls: Stall[]; featured: Stall }) {
...
function FeaturedStallCard({ stall, className = "" }: { stall: Stall; className?: string }) {
...
function StallTile({ stall }: { stall: Stall }) {
  const { bg, fg } = accentClasses(stall.accent);
  return (
    <Link
      href={`/stalls/${stall.slug}`}
